...
const outputPath = path.join(__dirname, "公文格式控制台.docm");
...
const selectionCommands = [
  ...
  { label: "公文页码", macro: "ConsoleApplyPageNumberFormat" },
];
...
function createDocument() {
  return new Document({
    creator: "Codex",
    title: "公文格式控制台",
    description: "嵌入 WPS JSA 宏和可执行 MacroButton 字段的方案 B 控制台",
    styles: {
      default: {
        document: {
    ...
    sections: [
      {
        ...
        children: [
          ...
          textParagraph("将本 DOCM 与待处理公文放在同一文件夹", {
            size: 19,
            color: colors.gray,
            alignment: AlignmentType.CENTER,
            after: 100,
          ...
          textParagraph(
            "按钮使用高光和阴影边框；多文件时自动打开原生文件选择框，可用上下方向键选择并按 Enter。仅在选择框不可用时输入序号。WPS 的 MacroButton 通常需要双击。",
            {
              size: 16,
              color: colors.gray,
              before: 90,
